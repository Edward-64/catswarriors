...
class Cookie {
	...
	setCookie(rawData, res, onlyset) {
		try {
	      	...
	      	for (let i = 1; i <= this.db.totalCats; i++) {
					...
					res.setHeader('set-cookie', [`auth=${this.db.cats[i].cookie};expires=${expires};httponly=true;SameSite=lax`,
					`alias=${encodeURI(this.db.cats[i].alias)};expires=${expires};httponly=true;SameSite=lax`]);
		            	if (!onlyset) {
	...
	deleteCookie(res, onlydel) {
		res.setHeader('set-cookie', ['alias=;max-age=0;SameSite=lax', 'timealias=;max-age=0;SameSite=lax']);
		if (!onlydel) res.end();
	}
