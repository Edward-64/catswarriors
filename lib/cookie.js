...
class Cookie {
	...
	setCookie(rawData, res, onlyset) {
		try {
	      	...
	      	for (let i = 1; i <= this.db.totalCats; i++) {
					...
					res.setHeader('set-cookie', [`auth=${this.db.cats[i].cookie};expires=${expires};Httponly=true;SameSite=Strict`,
					`alias=${encodeURI(this.db.cats[i].alias)};expires=${expires};Httponly=true;SameSite=Strict`]);
		            	if (!onlyset) {
	...
	deleteCookie(res, onlydel) {
		res.setHeader('set-cookie', 'alias=;max-age=0;Httponly=true;SameSite=Strict');
		if (!onlydel) res.end();
	}
