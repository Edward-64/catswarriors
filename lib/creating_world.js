...
class CreatingWorld extends require('events').EventEmitter {
	...
	getNamesExDetails(dir) {
		fs.readdir(dir + `/img/details/`, (err, dirs) => {
			if (err) {
				validator.log(`ошибка при чтении директории default-объектов: ${err}`);
			...
			for(let i = 0; i < l; i++) {
				fs.readdir(dir + `/img/details/${dirs[i]}`, (err, files) => {
					if (err) {
						validator.log(`ошибка при чтении директории существующих объектов: ${err}`);
						return this.emit('finishGetNamesExDetails', true);
					}
	...
	getNamesDetails(dir) {
		fs.readdir(dir + `/img/details/`, (err, dirs) => {
			if (err) {
				validator.log(`ошибка при чтении директории: ${err}`);
			...
			for(let i = 0; i < l; i++) {
				fs.readFile(dir + `/img/details/` + dirs[i], 'utf8', (err, data) => {
					if (err) {
						validator.log(`ошибка при чтении файла: ${err}`);
						return this.emit('finishGetNamesDetails', true);
					}
					let d = data.match(/#[\dABCDEF]{6}/ig);
					if (!d) { validator.log('ошибка при вычислении всевозможных палитр'); return this.emit('finishGetNamesDetails', true); };
					d = d.filter((x, j) => {
